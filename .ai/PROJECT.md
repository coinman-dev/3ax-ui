...
**3AX-UI** — форк панели управления [3x-ui](https://github.com/mhsanaei/3x-ui), переработанный и переименованный.  
...
Отдельный HTTP-сервер на порту **2096**.

- Биндится одновременно на IPv4 (`0.0.0.0:2096`) и IPv6 (`[::]:2096`)
- `sub/sub.go` — старт, роутер, dual-stack
- `sub/subService.go` — генерация ссылок, `ResolveRequest`, `BuildURLs`, `BuildPageData`
- `sub/subController.go` — хендлеры

### Важные особенности IPv6

`net.SplitHostPort("[2a10::1]:2096")` → возвращает `"2a10::1"` **без скобок**.  
Для URI-формата IPv6 нужны скобки. Используй `wrapIPv6(addr)` из `subService.go`.

`net.JoinHostPort("2a10::1", "2096")` → возвращает `"[2a10::1]:2096"` ✓  
`net.JoinHostPort("[2a10::1]", "2096")` → возвращает `"[[2a10::1]]:2096"` ✗ — не передавай адрес со скобками.

---

## Настройки приложения

Все настройки — в `web/service/setting.go`.  
Дефолтные значения: `var defaultValueMap = map[string]string{...}` (~строка 27).

Ключевые настройки:

| Ключ | По умолчанию | Описание |
|------|-------------|----------|
| `webPort` | `2053` | Порт панели |
| `subPort` | `2096` | Порт сервиса подписок |
| `subTheme` | `dark` | Тема страницы подписки |
| `subDomain` | `""` | Домен/IP для подписок (если пусто — берётся из запроса) |
| `subEnable` | `true` | Включить сервис подписок |
| `subEncrypt` | `true` | Base64-кодирование подписок |

Добавление новой настройки:
1. Добавить в `defaultValueMap`
2. Добавить `GetXxx()` / `SetXxx()` методы
3. Добавить в `GetDefaultSettings()` для отображения в UI
4. Добавить в шаблон настроек (`web/html/settings/panel/...`)
...
4. Затрагиваем ли мы сервис подписок (sub/) — там особенности с IPv6?
